...
// GNOME accent color names
type AccentColorName = 'blue' | 'teal' | 'green' | 'yellow' | 'orange' | 'red' | 'pink' | 'purple' | 'slate';
...
interface BackgroundColors {
    ...
    cardBg: string;
    cardBgActive: string;
    cardBgTemplate: string;
    toolbarBg: string;
    helpBoxBg: string;
    canvasBg: string;
}

// Visual depth colors (sections, inputs, dividers)
interface DepthColors {
    ...
    inputBg: string;
    inputBorder: string;
    inputShadowInset: string;
    divider: string;
}

// Card colors (workspace, monitor cards)
interface CardColors {
    workspaceCardBg: string;
    workspaceCardBgActive: string;
    monitorCardBg: string;
...
export class ThemeManager {
    ...
    private _signalIds: SignalTracker[];
     ...
     * @returns Background color properties
     */
    private _getBackgroundColors(isDark: boolean): BackgroundColors {
        return {
     ...
     * @param isDark - Whether dark mode is active
     * @returns Visual depth color properties
     */
    private _getDepthColors(isDark: boolean): DepthColors {
        return {
     ...
     * @param isDark - Whether dark mode is active
     * @returns Card color properties
     */
    private _getCardColors(isDark: boolean): CardColors {
        return {
     ...
     * Get text colors
     * @param isDark - Whether dark mode is active
     * @returns Text color properties
     */
    private _getTextColors(isDark: boolean): TextColors {
        return {
     ...
     * Get border colors
     * @param isDark - Whether dark mode is active
     * @returns Border color properties
     */
    private _getBorderColors(isDark: boolean): BorderColors {
        return {
     ...
     * @param isDark - Whether dark mode is active
     * @returns Button/menu color properties
     */
    private _getUIColors(isDark: boolean): UIColors {
        return {
     ...
     * @param accent - RGB accent color object
     * @param isDark - Whether dark mode is active
     * @returns Accent color properties
     */
    private _getAccentDerivedColors(accent: RGB, isDark: boolean): AccentDerivedColors {
     ...
     * @returns Color palette with all UI colors
     */
     ...
     * @returns Object with signal IDs for cleanup
     */
    connectChanged(callback: () => void): ThemeSignalIds {
     ...
     * @param ids - Object with userPrefId and systemId
     */
    disconnectChanged(ids: ThemeSignalIds): void {
     ...
     * @returns RGB color object in 0-1 range
     */
    private _getAccentColor(): RGB {
        try {
            const accentColorName = this._interfaceSettings.get_string('accent-color') as AccentColorName;

            const accentColors: Record<AccentColorName, RGB> = {
     ...
     * @param rgb - RGB object in 0-1 range
     * @param alpha - Alpha value (0-1)
     * @returns RGBA color string
     */
    private _rgbaWithAlpha(rgb: RGB, alpha: number): string {
    ...
    destroy(): void {
        ...
        (this._interfaceSettings as any) = null;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (this._settings as any) = null;
